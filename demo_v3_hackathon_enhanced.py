...
class TrainConfig:
    ...
    # Optional LOCAL-CONSISTENCY augmentation.
    # This does NOT invent fake CFD labels. Small perturbations are applied only to the
    # four continuous heat-load inputs, and the model is softly encouraged not to jump
    # discontinuously for nearly identical operating conditions. Default=0 keeps the
    # original training behavior exactly unchanged.
    consistency_weight: float = 0.0
    consistency_noise_std: float = 0.03
...
def local_condition_consistency_loss(
    model: PopFieldHVACTwin,
    cond_norm: torch.Tensor,
    coords_norm_t: torch.Tensor,
    base_field_pred: torch.Tensor,
    base_ra_pred: torch.Tensor,
    noise_std: float,
    cond_norm_min: Optional[torch.Tensor] = None,
    cond_norm_max: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """Optional safe-ish local smoothness regularizer for sparse CFD design points.

    We perturb ONLY the four heat-load dimensions (external/meeting/server/working).
    Direction bits and HVAC action variables (CMM/supply temperature) are left untouched.
    No pseudo CFD target is created. Instead, nearby inputs are softly constrained to avoid
    implausibly abrupt prediction jumps. This is a regularizer, not new physical ground truth,
    so it must be validated with the original validation/test split before claiming improvement.
    """
    if float(noise_std) <= 0:
        return base_field_pred.new_zeros(())

    pert = cond_norm.clone()
    dims = [3, 4, 5, 6]
    noise = torch.randn_like(pert[:, dims]) * float(noise_std)
    pert[:, dims] = pert[:, dims] + noise

    if cond_norm_min is not None and cond_norm_max is not None:
        lo = cond_norm_min[dims].view(1, -1)
        hi = cond_norm_max[dims].view(1, -1)
        pert[:, dims] = torch.maximum(torch.minimum(pert[:, dims], hi), lo)

    pert_field, pert_ra = model(pert, coords_norm_t)
    field_cons = F.smooth_l1_loss(pert_field, base_field_pred.detach())
    ra_cons = F.smooth_l1_loss(pert_ra, base_ra_pred.detach())
    return field_cons + 0.30 * ra_cons
...
def train_model(
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for cond, field, ra, _ in dl_train:
            ...
            pf, pr = model(cond, coords_norm_t)
            supervised_loss, _parts = loss_fn(
                pf, field, pr, ra, cfg.velocity_weight, cfg.ra_weight
            )

            cons_loss = pf.new_zeros(())
            if float(cfg.consistency_weight) > 0:
                cons_loss = local_condition_consistency_loss(
                    model=model,
                    cond_norm=cond,
                    coords_norm_t=coords_norm_t,
                    base_field_pred=pf,
                    base_ra_pred=pr,
                    noise_std=float(cfg.consistency_noise_std),
                    cond_norm_min=cond_norm_min,
                    cond_norm_max=cond_norm_max,
                )

            loss = supervised_loss + float(cfg.consistency_weight) * cons_loss
            ...
            consistency_losses.append(float(cons_loss.detach().cpu()))
        ...
        consistency_loss_mean = (
            float(np.mean(consistency_losses)) if consistency_losses else 0.0
        )

        row = {
            "epoch": epoch,
            "train_loss_norm": train_loss,
            "train_consistency_loss": consistency_loss_mean,
        ...
        cons_txt = (
            f" | cons={consistency_loss_mean:.5f}"
            if float(cfg.consistency_weight) > 0 else ""
        )
        print(
            f"Epoch {epoch:03d} | train={train_loss:.5f}{cons_txt} | "
...
def _select_deploy_monitor_indices(conditions: np.ndarray, num_cases: int = 3) -> np.ndarray:
    """Pick fixed low/mid/high total-heat examples for human-readable deployment monitoring.

    These cases are part of the 200 deployment-fitting scenarios, so the printed numbers are
    TRAIN-FIT diagnostics only, not validation/test/generalization scores.
    """
    conditions = np.asarray(conditions, dtype=np.float32)
    if len(conditions) == 0:
        return np.asarray([], dtype=np.int64)

    k = max(1, min(int(num_cases), len(conditions)))
    total_heat = conditions[:, 3:7].sum(axis=1)
    ...
    selected = []
    order = np.argsort(total_heat)
    for q in targets:
        pos = int(round(float(q) * (len(order) - 1)))
        idx = int(order[pos])
        if idx not in selected:
            selected.append(idx)

    # Fill any duplicate-quantile gaps deterministically.
    if len(selected) < k:
...
def _hvac_direction_label(cond_row: np.ndarray) -> str:
    names = []
    if int(round(float(cond_row[0]))) == 1:
        names.append("L")
    if int(round(float(cond_row[1]))) == 1:
        names.append("M")
    if int(round(float(cond_row[2]))) == 1:
        names.append("R")
    return "+".join(names) if names else "OFF"


@torch.no_grad()
def print_deploy_training_monitor(
    model: PopFieldHVACTwin,
    epoch: int,
    epochs: int,
    ...
    """Print fixed representative CFD cases during train_deploy.

    IMPORTANT: These are training-fit examples selected from the same 200 CFD cases used for
    deployment fitting. They help a human see whether learning is behaving sensibly, but they
    must never be reported as validation/test/generalization performance.
    """
    indices = np.asarray(monitor["indices"], dtype=np.int64)
    if len(indices) == 0:
        return

    conditions = np.asarray(monitor["conditions"], dtype=np.float32)
    fields = np.asarray(monitor["fields"], dtype=np.float32)
    ra_true_all = np.asarray(monitor["ra"], dtype=np.float32)
    dp_ids = np.asarray(monitor["dp_ids"], dtype=np.int64)
    cond_scaler: Standardizer = monitor["cond_scaler"]  # type: ignore[assignment]
    field_scaler: Standardizer = monitor["field_scaler"]  # type: ignore[assignment]
    ra_scaler: Standardizer = monitor["ra_scaler"]  # type: ignore[assignment]

    cond_raw = conditions[indices]
    cond_norm = cond_scaler.transform(cond_raw).astype(np.float32)
    cond_t = torch.from_numpy(cond_norm).to(device)

    was_training = model.training
    model.eval()
    pred_field_norm, pred_ra_norm = model(cond_t, coords_norm_t)
    pred_field = to_numpy(field_inverse_torch(pred_field_norm, field_scaler))
    pred_ra = to_numpy(ra_inverse_torch(pred_ra_norm, ra_scaler))
    if was_training:
        model.train()

    true_field = fields[indices]
    true_ra = ra_true_all[indices]

    temp_mae = np.mean(np.abs(pred_field[..., 0] - true_field[..., 0]), axis=1)
    mean_true = np.mean(true_field[..., 0], axis=1)
    mean_pred = np.mean(pred_field[..., 0], axis=1)
    p95_true = np.percentile(true_field[..., 0], 95, axis=1)
    p95_pred = np.percentile(pred_field[..., 0], 95, axis=1)

    avg_mae = float(np.mean(temp_mae))
    previous = monitor.get("_previous_avg_mae", None)
    if previous is None:
        trend = "baseline"
        delta = float("nan")
    else:
        delta = avg_mae - float(previous)
        if delta < -1e-4:
            trend = f"improved {abs(delta):.3f}C"
        elif delta > 1e-4:
            trend = f"worse +{delta:.3f}C"
        else:
            trend = "stable"

    monitor["_previous_avg_mae"] = avg_mae

    print("\n" + "-" * 108)
    print(f"[DEPLOY TRAIN-FIT MONITOR | Epoch {epoch:03d}/{epochs:03d}]")
    print("NOTE: fixed examples from the 200 training CFD cases; this is NOT validation/test performance.")
    print(f"Representative-case mean temperature MAE = {avg_mae:.4f} C | trend vs previous monitor: {trend}")
    print("-" * 108)

    history = monitor.setdefault("history", [])
    heat_totals = cond_raw[:, 3:7].sum(axis=1)
    heat_min = float(np.min(np.asarray(monitor["conditions"])[:, 3:7].sum(axis=1)))
    heat_max = float(np.max(np.asarray(monitor["conditions"])[:, 3:7].sum(axis=1)))

    for j, idx in enumerate(indices):
        c = cond_raw[j]
        total_heat = float(heat_totals[j])
        if abs(total_heat - heat_min) < 1e-6:
            load_tag = "LOW-LOAD"
        elif abs(total_heat - heat_max) < 1e-6:
            load_tag = "HIGH-LOAD"
        else:
            load_tag = "MID-LOAD"

        direction = _hvac_direction_label(c)
        print(
            f"[{j+1}/{len(indices)}] {load_tag} | DP {int(dp_ids[idx])} | "
            f"HVAC={direction}, CMM={float(c[7]):.0f}, Supply={float(c[8]):.0f}C"
        )
        print(
            f"      Heat loads [W] | external={float(c[3]):.0f} | meeting={float(c[4]):.0f} | "
            f"server={float(c[5]):.0f} | working={float(c[6]):.0f} | total={total_heat:.0f}"
        )
        print(
            f"      Temperature    | MAE={float(temp_mae[j]):.4f}C | "
            f"Mean actual/pred={float(mean_true[j]):.2f}/{float(mean_pred[j]):.2f}C | "
            f"P95 actual/pred={float(p95_true[j]):.2f}/{float(p95_pred[j]):.2f}C"
        )
        print(
            f"      RA temperature | actual/pred={float(true_ra[j]):.2f}/{float(pred_ra[j]):.2f}C"
        )

        history.append({
            "epoch": int(epoch),
            "monitor_case_order": int(j + 1),
            "load_tag": load_tag,
            "dp_id": int(dp_ids[idx]),
            "Inlet_L": int(round(float(c[0]))),
            "Inlet_M": int(round(float(c[1]))),
            "Inlet_R": int(round(float(c[2]))),
            "external_W": float(c[3]),
            "meeting_W": float(c[4]),
            "server_W": float(c[5]),
            "working_W": float(c[6]),
            ...
            "CMM": float(c[7]),
            "AirTemp_C": float(c[8]),
            "temperature_mae_C": float(temp_mae[j]),
            "actual_mean_temp_C": float(mean_true[j]),
            "pred_mean_temp_C": float(mean_pred[j]),
            "actual_p95_temp_C": float(p95_true[j]),
            "pred_p95_temp_C": float(p95_pred[j]),
            "actual_RA_temp_C": float(true_ra[j]),
            "pred_RA_temp_C": float(pred_ra[j]),
            "representative_mean_mae_C": avg_mae,
        })
...
def train_model_fixed_epochs(
    ...
    device: str,
    epochs: int,
    lr_schedule: Optional[Sequence[float]] = None,
    monitor: Optional[Dict[str, object]] = None,
    cond_norm_min: Optional[torch.Tensor] = None,
    cond_norm_max: Optional[torch.Tensor] = None,
) -> Tuple[Dict[str, torch.Tensor], List[Dict[str, float]]]:
    """Final deployment training on ALL available CFD scenarios.

    Validation/test model selection must be completed before this stage. Optional local
    consistency regularization can be reused only after its weight/noise were selected in
    the split-based evaluation run; deployment fitting itself must not tune them again.
    """
    optimizer = torch.optim.AdamW(model.parameters(), lr=cfg.lr, weight_decay=cfg.weight_decay)
    history: List[Dict[str, float]] = []
    epochs = int(epochs)
    if epochs <= 0:
        raise ValueError("deploy epochs must be > 0")

    for epoch in range(1, epochs + 1):
        if lr_schedule is not None and len(lr_schedule) >= epoch:
            lr_now = float(lr_schedule[epoch - 1])
        ...
        print(
            f"Deploy Epoch {epoch:03d}/{epochs:03d} | train={train_loss:.5f}{cons_txt} | "
            f"lr={lr_now:.2e} | {elapsed:.1f}s"
        )

        if monitor is not None:
            every = max(0, int(monitor.get("every", 0)))
            should_monitor = every > 0 and (epoch == 1 or epoch % every == 0 or epoch == epochs)
            if should_monitor:
                print_deploy_training_monitor(
                    model=model,
                    epoch=epoch,
                    epochs=epochs,
                    monitor=monitor,
...
def load_sensor_reconstruction_basis(
    path: str | Path,
    coords: Optional[np.ndarray] = None,
) -> Dict[str, object]:
    """Load PCA/QR sparse-sensor reconstruction assets.

    This NPZ is an operational current-state reconstruction asset. Loading it does
    not retrain or alter the PopField checkpoint.
    """
    path = Path(path)
    if not path.exists():
        raise FileNotFoundError(path)
    data = np.load(path, allow_pickle=False)
    required = {"temperature_mean", "temperature_basis", "selected_sensor_idx", "ridge"}
    missing = sorted(required.difference(data.files))
    if missing:
        raise ValueError(f"Sensor basis file missing arrays: {missing}")

    mean_field = np.asarray(data["temperature_mean"], dtype=np.float32)
    basis = np.asarray(data["temperature_basis"], dtype=np.float32)
    sensor_idx = np.asarray(data["selected_sensor_idx"], dtype=np.int64).reshape(-1)
    ridge_arr = np.asarray(data["ridge"], dtype=np.float32).reshape(-1)
    ridge = float(ridge_arr[0]) if len(ridge_arr) else 1e-3

    if mean_field.ndim != 1 or basis.ndim != 2 or basis.shape[0] != len(mean_field):
        raise ValueError("Invalid sensor reconstruction basis shapes")
    if np.any(sensor_idx < 0) or np.any(sensor_idx >= len(mean_field)):
        raise ValueError("selected_sensor_idx contains an out-of-range node index")
    if coords is not None and len(coords) != len(mean_field):
        raise ValueError(
            f"Sensor basis nodes ({len(mean_field)}) do not match checkpoint coordinates ({len(coords)})"
        )

    return {
        "temperature_mean": mean_field,
        "temperature_basis": basis,
        "selected_sensor_idx": sensor_idx,
        "ridge": ridge,
        "num_sensors": int(len(sensor_idx)),
        "path": str(path),
    }


def reconstruct_current_temperature_from_sensors(
    sensor_values_c: Sequence[float],
    sensor_basis_path: str | Path,
    coords: Optional[np.ndarray] = None,
) -> Tuple[np.ndarray, Dict[str, object]]:
    """Reconstruct the CURRENT full temperature field from sparse measured sensors."""
    assets = load_sensor_reconstruction_basis(sensor_basis_path, coords=coords)
    values = np.asarray(list(sensor_values_c), dtype=np.float32).reshape(1, -1)
    idx = np.asarray(assets["selected_sensor_idx"], dtype=np.int64)
    if values.shape[1] != len(idx):
        raise ValueError(
            f"Expected {len(idx)} sensor temperatures from the sensor basis, got {values.shape[1]}"
        )
    if not np.isfinite(values).all():
        raise ValueError("Sensor temperatures contain NaN/Inf")
    recon = reconstruct_from_sparse(
        values, idx,
        np.asarray(assets["temperature_mean"], dtype=np.float32),
        np.asarray(assets["temperature_basis"], dtype=np.float32),
        ridge=float(assets["ridge"]),
    )[0].astype(np.float32)
    info = {
        "sensor_basis_path": str(sensor_basis_path),
        "selected_sensor_idx": idx.tolist(),
        "sensor_values_C": values[0].astype(float).tolist(),
        "num_sensors": int(len(idx)),
        "ridge": float(assets["ridge"]),
    }
    return recon, info
...
def optimize_hvac(
    ...
    Hotspot safety ("do no harm" relative to the current HVAC action):
      5) do not create a new hotspot above the comfort upper bound
      6) do not worsen nodes that are already hotspots by more than tolerance
      7) do not increase the spatial maximum temperature by more than tolerance

    If no action satisfies every comfort constraint while remaining hotspot-safe,
    fallback selection is restricted to hotspot-safe actions first. The current HVAC
    action is itself a safe reference, so the recommender does not need to choose a
    locally harmful action merely to improve an aggregate score.
    """
    if max_p95_temp_c is None:
        max_p95_temp_c = float(target_temp_c + comfort_band_c)
    ...
    # Compare every candidate against the CURRENT HVAC action under
    # exactly the same user-entered heat loads.
    # ------------------------------------------------------------
    current_row = find_current_case(case_df)
    current_action = (
        int(current_row["P80 - Inlet L"]),
        int(current_row["P81 - Inlet M"]),
        int(current_row["P82 - Inlet R"]),
        float(current_row["P87 - CMM"]),
        float(current_row["P88 - AirTemp"]),
    )

    current_idx = None
    for j, a in enumerate(actions):
        if (
            int(a[0]) == current_action[0]
            and int(a[1]) == current_action[1]
            and int(a[2]) == current_action[2]
            and abs(float(a[3]) - current_action[3]) < 1e-6
            and abs(float(a[4]) - current_action[4]) < 1e-6
        ):
            current_idx = int(j)
            break
    ...
    if current_idx is not None:
        ...
        current_cond = np.asarray([[
            current_action[0], current_action[1], current_action[2],
            loads["external"], loads["meeting"], loads["server"], loads["working"],
            current_action[3], current_action[4],
        ]], dtype=np.float32)
    ...
    if current_temp_override is None:
        ...
        current_state_source = "popfield_current_estimate"
    else:
        current_temp = np.asarray(current_temp_override, dtype=np.float32).reshape(-1)
        if len(current_temp) != len(coords):
            raise ValueError(
                f"current_temp_override length {len(current_temp)} != number of nodes {len(coords)}"
            )
    ...
    for i, action in enumerate(actions):
        raw_temp = field[i, :, 0]
        # With real sensor input, anchor the current absolute field to the sensor
        # reconstruction and use PopField only for the candidate-vs-current HVAC delta.
        if current_temp_override is None:
            temp = raw_temp
        else:
        ...
        # We do NOT forbid all warming: a previously over-cooled location may
        # appropriately warm toward the comfort band. We only prohibit harmful warming.
        # --------------------------------------------------------
        new_hotspot_mask = (current_temp <= upper) & (temp > upper)
        new_hotspot_count = int(np.sum(new_hotspot_mask))
        new_hotspot_fraction = float(np.mean(new_hotspot_mask))

        if bool(np.any(current_hot_mask)):
            existing_hotspot_max_increase = float(
                np.max(temp[current_hot_mask] - current_temp[current_hot_mask])
            )
        else:
            existing_hotspot_max_increase = 0.0

        max_temp_change_vs_current = float(np.max(temp) - current_max_temp)

        no_new_hotspot_ok = new_hotspot_count == 0
        ...
        row = {
            ...
            "cold_fraction_constraint_met": bool(cold_ok),
            "p95_constraint_met": bool(p95_ok),
            "comfort_constraint_met": bool(comfort_ok),
            "hotspot_safety_tolerance_C": float(safety_tol),
            "new_hotspot_count": int(new_hotspot_count),
            "new_hotspot_fraction": float(new_hotspot_fraction),
            "existing_hotspot_max_increase_C": float(existing_hotspot_max_increase),
            "max_temp_change_vs_current_C": float(max_temp_change_vs_current),
            "no_new_hotspot_constraint_met": bool(no_new_hotspot_ok),
    ...
    feasible = df[df["recommendation_constraint_met"]].copy()
    ...
    if has_feasible:
        ...
        recommendations = {
            ...
            "additional_capacity_estimate": {
                ...
                "interpretation": "At least one hotspot-safe candidate satisfies all configured comfort constraints."
            },
...
def run_train_deploy(args) -> None:
    """Train the final app/deployment checkpoint on ALL available CFD scenarios.

    IMPORTANT:
      - Use this only AFTER the evaluation protocol has already fixed architecture/hyperparameters.
      - No validation/test score is produced here; reported fit metrics are training-fit diagnostics only.
      - target_temp / external_level / meeting_level / server_level / working_level / demo_policy are
        NOT training filters. They remain user-time query inputs for demo/recommend modes.
    """
    ...
    all_idx = np.arange(n_cases, dtype=np.int64)

    # Final deployment scalers are fit on all available CFD scenarios.
    cond_scaler = Standardizer.fit(conditions, axis=0)
    coord_scaler = Standardizer.fit(coords, axis=0)
    field_scaler = Standardizer.fit(fields, axis=(0, 1))
    ra_scaler = Standardizer.fit(ra, axis=0)
    ...
    ds_all = CFDDataset(conditions, fields, ra, all_idx, cond_scaler, field_scaler, ra_scaler)
    dl_all = DataLoader(
        ds_all, batch_size=cfg.batch_size, shuffle=True, num_workers=cfg.num_workers
    )
    dl_all_eval = DataLoader(
        ds_all, batch_size=cfg.batch_size, shuffle=False, num_workers=cfg.num_workers
    )
    ...
    cond_norm_all = cond_scaler.transform(conditions).astype(np.float32)
    cond_norm_min_t = torch.from_numpy(cond_norm_all.min(axis=0)).to(device)
    cond_norm_max_t = torch.from_numpy(cond_norm_all.max(axis=0)).to(device)
    ...
    lr_schedule = None
    ref_history = getattr(args, "deploy_reference_history", None)
    if ref_history:
        ref_path = Path(ref_history)
        if not ref_path.exists():
            raise FileNotFoundError(f"deploy reference history not found: {ref_path}")
        h = pd.read_csv(ref_path)
        if "lr" not in h.columns:
            raise ValueError("deploy reference history must contain an 'lr' column")
        lr_schedule = h["lr"].astype(float).tolist()
        print(f"[Deploy] Reusing LR schedule from: {ref_path}")

    monitor_every = max(0, int(getattr(args, "deploy_monitor_every", 10)))
    monitor_cases = max(1, int(getattr(args, "deploy_monitor_cases", 3)))
    monitor_indices = _select_deploy_monitor_indices(conditions, num_cases=monitor_cases)
    deploy_monitor: Optional[Dict[str, object]] = None
    if monitor_every > 0:
        deploy_monitor = {
            "every": monitor_every,
    ...
    hr()
    print("PopField HVAC — FINAL DEPLOYMENT TRAINING")
    print(f"Device={device} | ALL CFD cases={n_cases} | nodes={n_nodes} | params={count_params(model):,}")
    print(f"Training split: {n_cases}/{n_cases} cases (100% used for final deployment fitting)")
    print(f"Fixed epochs: {deploy_epochs} (chosen from prior evaluation; no early stopping here)")
    print("User demo inputs such as target_temp/medium/high are NOT used to filter this training set.")
    if float(cfg.consistency_weight) > 0:
        print(f"Local consistency augmentation: ON | weight={cfg.consistency_weight:g} | normalized heat-load noise std={cfg.consistency_noise_std:g}")
    else:
        print("Local consistency augmentation: OFF (original CFD supervision only)")
    if deploy_monitor is not None:
        print(
            f"Training monitor: every {monitor_every} epoch(s) | "
            f"fixed representative CFD cases={monitor_indices.tolist()} "
            f"(low/mid/high heat-load coverage; TRAIN-FIT only)"
        )
    else:
        print("Training monitor: disabled")
    hr()

    final_state, history = train_model_fixed_epochs(
        model, dl_all, coords_norm_t, cfg, device, deploy_epochs,
        lr_schedule=lr_schedule, monitor=deploy_monitor,
        cond_norm_min=cond_norm_min_t, cond_norm_max=cond_norm_max_t,
    )
    model.load_state_dict(final_state)

    # Training-fit diagnostics only. These are NOT held-out generalization metrics.
    fit_pred, fit_true, fit_ra_pred, fit_ra_true, _ = predict_loader(
        model, dl_all_eval, coords_norm_t, field_scaler, ra_scaler, device
    )
    fit_metrics = metrics_from_arrays(fit_pred, fit_true, fit_ra_pred, fit_ra_true)

    all_metrics = {
        "deployment_training": {
            "num_cfd_cases_used": int(n_cases),
            "fixed_epochs": int(deploy_epochs),
            "all_data_used_for_fitting": True,
            "fit_metrics_not_generalization_metrics": True,
            "fit_metrics": fit_metrics,
            "reference_history": str(ref_history) if ref_history else None,
            "training_monitor_note": (
                "Representative cases are drawn from the same 200 deployment-fitting CFD scenarios; "
                "monitor values are training-fit diagnostics only, not validation/test/generalization scores."
            ),
            "training_monitor_every": int(monitor_every),
            "training_monitor_indices": monitor_indices.tolist() if deploy_monitor is not None else [],
        },
        "params": count_params(model),
    }

    ckpt_path = save_dir / "best_deploy.pt"
    save_checkpoint(
        ckpt_path, model, cfg, cond_scaler, coord_scaler, field_scaler, ra_scaler,
        coords, {"train": all_idx, "val": np.asarray([], dtype=np.int64), "test": np.asarray([], dtype=np.int64)},
        all_metrics,
    )
    pd.DataFrame(history).to_csv(save_dir / "deployment_training_history.csv", index=False)
    if deploy_monitor is not None:
        pd.DataFrame(deploy_monitor.get("history", [])).to_csv(
            save_dir / "deployment_monitor_history.csv", index=False
        )
    (save_dir / "deployment_result.json").write_text(
        json.dumps(all_metrics, indent=2, ensure_ascii=False), encoding="utf-8"
    )

    print("\n[Deployment training complete]")
    print(f"  ALL CFD cases used : {n_cases}")
    print(f"  Fixed epochs       : {deploy_epochs}")
    print(f"  Saved checkpoint   : {ckpt_path}")
    print("  NOTE: fit metrics above are NOT test/generalization scores.")
    print("  Use the earlier best.pt evaluation results when reporting model accuracy.")
...
def run_train(args) -> None:
    ...
    if float(cfg.consistency_weight) > 0:
        ...
        print("Local consistency augmentation: OFF")
    hr()
...
def _load_range_diagnostics(case_df: pd.DataFrame, loads: Dict[str, float]) -> Dict[str, Dict[str, object]]:
    """Describe whether exact/continuous heat-load inputs lie inside supplied CFD factor ranges."""
    col_map = {
        "external": "P83 - external",
        "meeting": "P84 - meeting",
        "server": "P85 - server",
    ...
    for key, col in col_map.items():
        ...
        exact_observed = bool(np.any(np.isclose(values, value, atol=1e-9)))
        inside = bool(lo - 1e-9 <= value <= hi + 1e-9)
        nearest = float(values[np.argmin(np.abs(values - value))])
        lower_vals = values[values <= value]
        upper_vals = values[values >= value]
        lower = float(lower_vals[-1]) if len(lower_vals) else lo
        upper = float(upper_vals[0]) if len(upper_vals) else hi
        if exact_observed:
            mode = "observed_level"
        elif inside:
            mode = "interpolation_query"
        else:
            mode = "extrapolation_warning"
        out[key] = {
            "value_W": value,
            "observed_min_W": lo,
            "observed_max_W": hi,
            "exact_observed_level": exact_observed,
            "inside_observed_range": inside,
            "nearest_observed_W": nearest,
            "bracketing_observed_W": [lower, upper],
            "query_mode": mode,
        }
    return out


def _print_load_range_diagnostics(diag: Dict[str, Dict[str, object]]) -> None:
    print("\n[Input-range check]")
    for key in ["external", "meeting", "server", "working"]:
        d = diag[key]
        value = float(d["value_W"])
        lo = float(d["observed_min_W"])
        hi = float(d["observed_max_W"])
        if bool(d["exact_observed_level"]):
            print(f"  ✅ {key:8s}: {value:.0f} W | supplied CFD level")
        elif bool(d["inside_observed_range"]):
            b0, b1 = d["bracketing_observed_W"]
            print(
                f"  ◌  {key:8s}: {value:.0f} W | continuous interpolation query "
                f"(between observed {float(b0):.0f} and {float(b1):.0f} W)"
            )
        else:
            print(
                f"  ⚠  {key:8s}: {value:.0f} W | OUTSIDE learned range {lo:.0f}~{hi:.0f} W; "
                "prediction uncertainty may increase"
            )
...
def _resolve_demo_inputs(case_df: pd.DataFrame, args) -> Tuple[Dict[str, float], Dict[str, str], str]:
    ...
    if bool(getattr(args, "demo_interactive", False)):
        ...
        if bool(getattr(args, "demo_continuous_inputs", False)):
            print("\n연속형 열부하 입력 모드: W 값을 직접 입력합니다.")
            prompt_names = {
                "external": "외부 열환경",
                "meeting": "회의공간",
                "server": "서버/전자기기",
            ...
            for key, col in col_map.items():
                values = np.sort(case_df[col].astype(float).unique())
                default_v = float(level_maps[key]["medium"])
                setattr(
                    args, key,
                    _prompt_float(
                        f"{prompt_names[key]} 열부하 W (CFD 관측범위 {values.min():.0f}~{values.max():.0f})",
                        default_v,
                    )
                )
        else:
...
def _predict_demo_action_field(
    model: PopFieldHVACTwin,
    action: Tuple[int, int, int, float, float],
    loads: Dict[str, float],
    scalers: Dict[str, Standardizer],
    ...
    cond = np.asarray([[
        ...
        float(loads["external"]), float(loads["meeting"]),
        float(loads["server"]), float(loads["working"]),
        float(cmm), float(supply),
    ]], dtype=np.float32)
    field, ra = predict_conditions(
        model, cond, scalers["cond"], coords_norm_t,
        scalers["field"], scalers["ra"], device, batch_size=1,
    )
    return field[0], float(ra[0])
...
def _save_demo_selected_field(
    ...
    """Save the recommended field, optionally anchored to measured current sensors."""
    action = (
        int(round(float(rec["Inlet_L"]))), int(round(float(rec["Inlet_M"]))),
        int(round(float(rec["Inlet_R"]))), float(rec["CMM"]), float(rec["AirTemp_C"]),
    )
    pred, ra = _predict_demo_action_field(model, action, loads, scalers, coords_norm_t, device)
    pred = pred.copy()
    if current_temp_override is not None:
        if case_df is None:
            raise ValueError("case_df is required when current_temp_override is supplied")
        current_row = find_current_case(case_df)
        current_action = (
            int(current_row["P80 - Inlet L"]), int(current_row["P81 - Inlet M"]),
            int(current_row["P82 - Inlet R"]), float(current_row["P87 - CMM"]),
            float(current_row["P88 - AirTemp"]),
        )
        model_cur, _ = _predict_demo_action_field(model, current_action, loads, scalers, coords_norm_t, device)
        sensor_cur = np.asarray(current_temp_override, dtype=np.float32).reshape(-1)
        pred[:, 0] = sensor_cur + (pred[:, 0] - model_cur[:, 0])
    ...
    out = pd.DataFrame({
        ...
        "pred_velocity_u": pred[:, 1], "pred_velocity_v": pred[:, 2],
        "pred_velocity_w": pred[:, 3], "pred_air_speed_mps": vel_mag,
        "pred_RA_temperature_C": float(ra),
    })
...
def _zone_name_for_node(zones: Dict[str, np.ndarray], idx: int) -> str:
...
def _select_spatially_distinct_nodes(
    coords: np.ndarray,
    score: np.ndarray,
    k: int,
    min_distance_m: float,
) -> List[int]:
    """Greedy non-maximum suppression so top hotspots are not five adjacent grid points."""
    ...
    for idx in order.tolist():
        ...
        if not selected:
            ...
            d = np.linalg.norm(coords[np.asarray(selected)] - coords[idx][None, :], axis=1)
            if bool(np.all(d >= float(min_distance_m))):
                selected.append(idx)
...
def build_demo_spatial_change_report(
    model: PopFieldHVACTwin,
    rec: Dict[str, object],
    loads: Dict[str, float],
    case_df: pd.DataFrame,
    ...
    current_temp_override: Optional[np.ndarray] = None,
    sensor_info: Optional[Dict[str, object]] = None,
) -> Dict[str, object]:
    """Compare AI-estimated CURRENT steady-state vs recommended steady-state at every node.

    CURRENT here means the currently defined HVAC action from Case Info evaluated under the
    user-entered heat loads. It is a model estimate unless actual sparse sensor measurements
    are separately supplied/reconstructed. Because the dataset is steady-state, the report
    never claims a time-to-cool trajectory.
    """
    current_row = find_current_case(case_df)
    current_action = (
        int(current_row["P80 - Inlet L"]), int(current_row["P81 - Inlet M"]),
    ...
    model_cur_field, cur_ra = _predict_demo_action_field(
        model, current_action, loads, scalers, coords_norm_t, device
    )
    model_new_field, new_ra = _predict_demo_action_field(
        model, rec_action, loads, scalers, coords_norm_t, device
    )
    cur_field = model_cur_field.copy()
    new_field = model_new_field.copy()

    if current_temp_override is None:
        cur_temp = model_cur_field[:, 0].copy()
        new_temp = model_new_field[:, 0].copy()
        current_state_source = "PopField estimate using Case Info current HVAC action + entered heat loads"
        interpretation = "steady-state before/after model estimate; not a time trajectory and not a direct sensor measurement"
    else:
        cur_temp = np.asarray(current_temp_override, dtype=np.float32).reshape(-1)
        if len(cur_temp) != len(coords):
            raise ValueError("current_temp_override length does not match coordinates")
        new_temp = cur_temp + (model_new_field[:, 0] - model_cur_field[:, 0])
        cur_field[:, 0] = cur_temp
        new_field[:, 0] = new_temp
        current_state_source = "Sparse sensor PCA/QR reconstruction of CURRENT temperature field"
        interpretation = (
            "current temperature field reconstructed from measured sparse sensors; "
            "recommended field = sensor-reconstructed current + PopField counterfactual HVAC delta; "
            "steady-state estimate, not a time trajectory"
        )

    delta = new_temp - cur_temp
    cooling = cur_temp - new_temp

    zones = build_zone_masks(coords, zone_json)
    p95_limit = (
        float(max_p95_temp_c) if max_p95_temp_c is not None
        else float(target_temp_c + comfort_band_c)
    )
    cur_metrics = field_comfort_metrics(
        cur_temp, cur_field[:, 1:4], cur_ra, current_action, zones,
        target_temp_c, comfort_band_c, max_zone_range_c,
        max_hot_fraction, max_cold_fraction, p95_limit,
    )
    new_metrics = field_comfort_metrics(
        new_temp, new_field[:, 1:4], new_ra, rec_action, zones,
        target_temp_c, comfort_band_c, max_zone_range_c,
        max_hot_fraction, max_cold_fraction, p95_limit,
    )

    hot_score = cur_temp.copy()
    hotspot_idx = _select_spatially_distinct_nodes(
        coords, hot_score, max(1, int(top_k)), max(0.0, float(min_distance_m))
    )
    hotspot_rows: List[Dict[str, object]] = []
    upper = float(target_temp_c + comfort_band_c)
    for rank, idx in enumerate(hotspot_idx, 1):
        hotspot_rows.append({
            "rank": int(rank), "node_index": int(idx),
            "zone": _zone_name_for_node(zones, idx),
            "x_m": float(coords[idx, 0]), "y_m": float(coords[idx, 1]), "z_m": float(coords[idx, 2]),
            "current_estimated_temp_C": float(cur_temp[idx]),
            "recommended_pred_temp_C": float(new_temp[idx]),
            "temperature_change_C": float(delta[idx]),
            "expected_cooling_C": float(cooling[idx]),
            "current_hotspot_above_band": bool(cur_temp[idx] > upper),
            "remaining_hotspot_above_band": bool(new_temp[idx] > upper),
        })

    hottest_idx = int(np.argmax(cur_temp))
    ...
    all_nodes = pd.DataFrame({
        ...
        "remaining_hotspot": new_temp > upper,
    })
    field_path = save_dir / "DEMO_CURRENT_VS_RECOMMENDED_FIELD.csv"
    all_nodes.to_csv(field_path, index=False)
    hotspot_path = save_dir / "DEMO_HOTSPOT_CHANGE.csv"
    pd.DataFrame(hotspot_rows).to_csv(hotspot_path, index=False)
    ...
    summary: Dict[str, object] = {
        ...
        "sensor_anchored_counterfactual": bool(current_temp_override is not None),
        "hot_threshold_C": float(upper),
        "current_action": {
            "Inlet_L": current_action[0], "Inlet_M": current_action[1], "Inlet_R": current_action[2],
            "CMM": current_action[3], "AirTemp_C": current_action[4],
        },
        "recommended_action": {
            "Inlet_L": rec_action[0], "Inlet_M": rec_action[1], "Inlet_R": rec_action[2],
            "CMM": rec_action[3], "AirTemp_C": rec_action[4],
        },
        "current_metrics": cur_metrics,
        "recommended_metrics": new_metrics,
        "changes": {
            "mean_temperature_change_C": float(new_metrics["mean_temp_C"] - cur_metrics["mean_temp_C"]),
            "max_temperature_change_C": float(new_metrics["max_temp_C"] - cur_metrics["max_temp_C"]),
            "p95_temperature_change_C": float(new_metrics["p95_temp_C"] - cur_metrics["p95_temp_C"]),
            "zone_range_change_C": float(new_metrics["zone_range_C"] - cur_metrics["zone_range_C"]),
            "hot_fraction_change_percentage_points": float(
                (new_metrics["hot_fraction"] - cur_metrics["hot_fraction"]) * 100.0
            ),
        },
        "hottest_current_location": {
            "node_index": hottest_idx,
            "zone": _zone_name_for_node(zones, hottest_idx),
            "xyz_m": coords[hottest_idx].astype(float).tolist(),
            "current_estimated_temp_C": float(cur_temp[hottest_idx]),
            "recommended_pred_temp_C": float(new_temp[hottest_idx]),
            "temperature_change_C": float(delta[hottest_idx]),
            "expected_cooling_C": float(cooling[hottest_idx]),
            "current_hotspot_above_band": bool(cur_temp[hottest_idx] > upper),
            "remaining_hotspot_above_band": bool(new_temp[hottest_idx] > upper),
        },
        "largest_expected_cooling_location": {
            "node_index": max_cooling_idx,
            "zone": _zone_name_for_node(zones, max_cooling_idx),
            "xyz_m": coords[max_cooling_idx].astype(float).tolist(),
            "current_estimated_temp_C": float(cur_temp[max_cooling_idx]),
            "recommended_pred_temp_C": float(new_temp[max_cooling_idx]),
            "expected_cooling_C": float(cooling[max_cooling_idx]),
        },
        "hottest_remaining_location": {
            "node_index": remaining_hottest_idx,
            "zone": _zone_name_for_node(zones, remaining_hottest_idx),
            "xyz_m": coords[remaining_hottest_idx].astype(float).tolist(),
            "recommended_pred_temp_C": float(new_temp[remaining_hottest_idx]),
        },
        "distinct_current_hotspots": hotspot_rows,
        "all_node_comparison_csv": str(field_path),
        "hotspot_summary_csv": str(hotspot_path),
    }
    summary_path = save_dir / "DEMO_SPATIAL_CHANGE.json"
    summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    summary["summary_json"] = str(summary_path)
    return summary


def _print_demo_spatial_change_report(report: Dict[str, object]) -> None:
    cur = report["current_metrics"]
    new = report["recommended_metrics"]
    hottest = report["hottest_current_location"]
    maxcool = report["largest_expected_cooling_location"]

    print("\n[AI-estimated current spatial diagnosis]")
    print("  ※ 실제 센서 실측값이 아니라, 현재 HVAC 설정 + 입력 열부하로 예측한 정상상태입니다.")
    highest_is_hotspot = bool(hottest.get("current_hotspot_above_band", False))
    highest_label = "🔥 현재 Hotspot" if highest_is_hotspot else "🌡️ 현재 최고온도 위치"
    print(
        f"  {highest_label}: {hottest['zone']} | node {hottest['node_index']} | "
        f"XYZ=({hottest['xyz_m'][0]:.2f}, {hottest['xyz_m'][1]:.2f}, {hottest['xyz_m'][2]:.2f}) m"
    )
    temp_change = float(hottest["recommended_pred_temp_C"]) - float(hottest["current_estimated_temp_C"])
    if temp_change < -0.01:
        change_text = f"예상 냉각 {abs(temp_change):.2f}C"
    elif temp_change > 0.01:
        change_text = f"예상 온도 상승 +{temp_change:.2f}C"
    else:
        change_text = "온도 변화 거의 없음"
    print(
        f"     {float(hottest['current_estimated_temp_C']):.2f}C -> "
        f"{float(hottest['recommended_pred_temp_C']):.2f}C | {change_text}"
    )
    print(
        f"  공간 평균       : {float(cur['mean_temp_C']):.2f}C -> {float(new['mean_temp_C']):.2f}C "
        f"({float(new['mean_temp_C'])-float(cur['mean_temp_C']):+.2f}C)"
    )
    print(
        f"  공간 최대온도   : {float(cur['max_temp_C']):.2f}C -> {float(new['max_temp_C']):.2f}C "
        f"({float(new['max_temp_C'])-float(cur['max_temp_C']):+.2f}C)"
    )
    print(
        f"  P95 온도        : {float(cur['p95_temp_C']):.2f}C -> {float(new['p95_temp_C']):.2f}C "
        f"({float(new['p95_temp_C'])-float(cur['p95_temp_C']):+.2f}C)"
    )
    print(
        f"  Zone 온도편차   : {float(cur['zone_range_C']):.2f}C -> {float(new['zone_range_C']):.2f}C "
        f"({float(new['zone_range_C'])-float(cur['zone_range_C']):+.2f}C)"
    )
    print(
        f"  Hot 영역        : {100*float(cur['hot_fraction']):.2f}% -> {100*float(new['hot_fraction']):.2f}%"
    )
    print(
        f"  ❄ 가장 큰 예상 냉각 위치: {maxcool['zone']} | "
        f"{float(maxcool['current_estimated_temp_C']):.2f}C -> {float(maxcool['recommended_pred_temp_C']):.2f}C "
        f"(냉각 {float(maxcool['expected_cooling_C']):.2f}C)"
    )

    print("\n[Top spatial hotspots — before -> after]")
    for h in report.get("distinct_current_hotspots", []):
        mark = "🔥" if bool(h["current_hotspot_above_band"]) else "•"
        remain = "⚠ still hot" if bool(h["remaining_hotspot_above_band"]) else "✅ relieved"
        print(
            f"  {mark} #{int(h['rank'])} {h['zone']:10s} node={int(h['node_index']):4d} | "
            f"{float(h['current_estimated_temp_C']):.2f}C -> {float(h['recommended_pred_temp_C']):.2f}C | "
            f"change={float(h['temperature_change_C']):+.2f}C | {remain}"
        )
...
def run_demo(args) -> None:
    ...
    input_range_diagnostics = _load_range_diagnostics(case_df, loads)
    ...
    spatial_change = build_demo_spatial_change_report(
        model=model, rec=rec, loads=loads, case_df=case_df, scalers=scalers,
        coords=coords, coords_norm_t=coords_norm_t, device=device, save_dir=save_dir,
        target_temp_c=float(args.target_temp), comfort_band_c=float(args.comfort_band),
        max_zone_range_c=float(args.max_zone_range),
        max_hot_fraction=float(args.max_hot_fraction), max_cold_fraction=float(args.max_cold_fraction),
        max_p95_temp_c=args.max_p95_temp, zone_json=args.zone_json,
        top_k=int(args.hotspot_top_k), min_distance_m=float(args.hotspot_min_distance_m),
    )
    ...
    result = {
        ...
        "spatial_change_analysis": spatial_change,
        "status": status,
        ...
        "notes": [
            ...
            "Consumer-friendly low/medium/high inputs are mapped to heat-load levels observed in Case Info; exact W inputs are also supported as continuous interpolation queries.",
    ...
    _print_load_range_diagnostics(input_range_diagnostics)
    ...
    print(f"  Before/after field: {spatial_change['all_node_comparison_csv']}")
    print(f"  Hotspot changes  : {spatial_change['hotspot_summary_csv']}")
    print(f"  Spatial summary  : {spatial_change['summary_json']}")
    print(f"  All 54 candidates: {save_dir / 'hvac_optimization.csv'}")
    print("\nNOTE: low/medium/high는 Case Info 관측 단계로 매핑되며, exact W 입력은 관측범위 안에서 연속 보간 질의로 사용할 수 있습니다.")
...
def run_sensor_export(args) -> None:
    """Export sparse-sensor basis/location files without retraining PopField."""
    save_dir = ensure_dir(args.save_dir)
    cache_path = save_dir / "sensor_export_cache.npz"
    data = build_or_load_cache(args.case_info, args.field_zip, cache_path, args.force_rebuild_cache)
    ...
    tr = va = te = None
    split_source = "deterministic_70_15_15_split"
    if args.checkpoint and Path(args.checkpoint).exists():
        ckpt = torch.load(args.checkpoint, map_location="cpu", weights_only=False)
        split = ckpt.get("split", {})
        tr0 = np.asarray(split.get("train", []), dtype=np.int64)
        va0 = np.asarray(split.get("val", []), dtype=np.int64)
        te0 = np.asarray(split.get("test", []), dtype=np.int64)
        if len(tr0) and len(va0) and len(te0):
            tr, va, te = tr0, va0, te0
            split_source = "checkpoint_split"
    if tr is None:
        tr, va, te = scenario_split(n, 0.70, 0.15, int(getattr(args, "seed", 42)))

    train_temp, val_temp, test_temp = fields[tr, :, 0], fields[va, :, 0], fields[te, :, 0]
    mean_temp, temp_basis, _svals = fit_pca_modes(train_temp, rank=int(args.sensor_pca_rank))
    rows, plans = [], {}
    counts = [int(x) for x in args.sensor_counts.split(",") if x.strip()]
    for k in counts:
        idx = select_sensors(temp_basis, k)
        val_recon = reconstruct_from_sparse(val_temp[:, idx], idx, mean_temp, temp_basis, ridge=float(args.sensor_ridge))
        test_recon = reconstruct_from_sparse(test_temp[:, idx], idx, mean_temp, temp_basis, ridge=float(args.sensor_ridge))
        rows.append({
            "num_sensors": k,
            "val_sensor_reconstruction_mae_c": float(np.mean(np.abs(val_recon - val_temp))),
            "test_sensor_reconstruction_mae_c": float(np.mean(np.abs(test_recon - test_temp))),
    ...
    table = pd.DataFrame(rows).sort_values("num_sensors").reset_index(drop=True)
    table.to_csv(save_dir / "sensor_study.csv", index=False)
    eligible = table[table["val_sensor_reconstruction_mae_c"] <= float(args.sensor_target_mae)]
    if len(eligible):
        chosen_k = int(eligible.sort_values("num_sensors").iloc[0]["num_sensors"])
        reason = f"smallest K with validation MAE <= {float(args.sensor_target_mae):.3f}C"
    else:
        chosen_k = int(table.sort_values(["val_sensor_reconstruction_mae_c", "num_sensors"]).iloc[0]["num_sensors"])
        reason = "target not reached; chose best validation reconstruction candidate"
    chosen_idx = plans[chosen_k]
    selected = pd.DataFrame({
        "sensor_order": np.arange(1, len(chosen_idx)+1),
        "node_index": chosen_idx,
        "x_m": coords[chosen_idx,0], "y_m": coords[chosen_idx,1], "z_m": coords[chosen_idx,2],
    })
    selected.to_csv(save_dir / "selected_sensors.csv", index=False)
    np.savez_compressed(
        save_dir / "sensor_reconstruction_basis.npz",
        temperature_mean=mean_temp, temperature_basis=temp_basis,
        selected_sensor_idx=chosen_idx, ridge=np.asarray([float(args.sensor_ridge)], dtype=np.float32),
    )
    chosen_row = table[table["num_sensors"] == chosen_k].iloc[0]
    plan = {
        "chosen_num_sensors": chosen_k, "selection_reason": reason,
        "chosen_node_indices": chosen_idx.tolist(),
        "chosen_validation_mae_c": float(chosen_row["val_sensor_reconstruction_mae_c"]),
        "chosen_test_mae_c": float(chosen_row["test_sensor_reconstruction_mae_c"]),
        "split_source": split_source,
    }
    (save_dir / "sensor_plan.json").write_text(json.dumps(plan, indent=2, ensure_ascii=False), encoding="utf-8")
    print("[Sensor export complete]")
    print(f"  chosen sensors : {chosen_k}")
    print(f"  basis           : {save_dir / 'sensor_reconstruction_basis.npz'}")
    print(f"  locations       : {save_dir / 'selected_sensors.csv'}")
    print(f"  validation MAE  : {plan['chosen_validation_mae_c']:.4f} C")
...
def parse_args():
    ...
    p.add_argument("--deploy_reference_history", type=str, default=None,
                   help="Optional prior training_history.csv; when supplied, train_deploy reuses its per-epoch learning-rate schedule")
    p.add_argument("--deploy_monitor_every", type=int, default=10,
                   help="In train_deploy, print fixed representative CFD prediction diagnostics every N epochs; 0 disables")
    p.add_argument("--deploy_monitor_cases", type=int, default=3,
                   help="Number of fixed low/mid/high heat-load CFD cases shown by the train_deploy console monitor")
    ...
    p.add_argument("--consistency_weight", type=float, default=0.0,
                   help="Optional local-consistency regularization weight. 0 keeps original training. Validate on val/test before deployment.")
    p.add_argument("--consistency_noise_std", type=float, default=0.03,
                   help="Normalized std of tiny perturbations applied only to heat-load inputs for local consistency")
    ...
    p.add_argument("--demo_interactive", action="store_true",
                   help="Prompt for target temperature, heat-load inputs and policy in demo mode")
    p.add_argument("--demo_continuous_inputs", action="store_true",
                   help="With --demo_interactive, ask exact continuous heat-load W values instead of low/medium/high")
    p.add_argument("--hotspot_top_k", type=int, default=5,
                   help="Number of spatially distinct current hotspots shown in demo before/after report")
    p.add_argument("--hotspot_min_distance_m", type=float, default=1.0,
                   help="Minimum distance between reported hotspot nodes so adjacent grid points are not repeated")
